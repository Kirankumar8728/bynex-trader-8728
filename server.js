...
// ============================================================================
app.get("/api/deriv/session", (req, res) => {
...
app.post("/api/deriv/logout", (req, res) => {
  if (req.session) {
    req.session.destroy(() => {
...
const handleUpdateWithdrawal = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    await sql`UPDATE withdrawals SET status = ${status}, rejectionReason = ${rejectionReason ? String(rejectionReason) : null} WHERE id = ${id}`;
...
app.get("/api/referral-balance/:userId", isAuthenticated, async (req, res) => {
  const { userId } = req.params;
  try {
    const { rows } = await sql`SELECT balance FROM balances WHERE userId = ${userId}`;
    if (rows.length > 0) {
...
app.post("/api/process-trade", async (req, res) => {
